...
hold on
plot(tImplicit,aImplicit,'linewidth',lw,'color',secColor);
hold off
xlabel('Time in seconds','Fontsize',16);
grid on
legend('uImplicit','aImplicit','Fontsize',16,'Location','northwest');
title('Kuo RK4','Fontsize',20);
...
title('Kuo Problem 2C','Fontsize',20);
legend(legendIDs,'location','northwest','box','off','Fontsize',16)
set(gca,'FontName',fn,'FontSize',fs,'linewidth',lw,'box','off')
...
tspan = [0,2];
y0 = 0.01;
[tODE45,aODE45] = ode45(@(t,a)muscleActODEfun(t,a), tspan, y0);
...
averageStepODE = 2/(length(tODE45)-1);
%2/(length(tODE45)-1); % ** Replace the brackets with code to calculate the
...
% ODE45
timespan = [0,2];
[t_bigStep, a_bigStep] = eulerSolver(@(t,a) muscleActODEfun(t,a),...
                        timespan, y0, averageStepODE); % ** Fill in the brackets with code to solve
...
hold off
grid on
legend({'implicit solution','ode45','euler'},'location','northwest','box','off','Fontsize',16)
title('Kuo Problem 2D','Fontsize',20);
set(gca,'FontName',fn,'FontSize',fs,'linewidth',lw,'box','off')